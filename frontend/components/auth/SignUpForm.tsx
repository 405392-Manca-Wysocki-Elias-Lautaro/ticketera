...
export const SignUpForm = ({ role, title }: SignUpFormProps) => {
    ...
    const onSubmit = async (data: SignUpSchema) => {
        try {
            // Force the role to match the prop, just in case
    ...
    return (
        <div className="h-screen w-full grid place-items-center p-4 gradient-brand overflow-y-auto">
            <Card className="w-full max-w-md gap-2 my-8">
                <CardHeader className="space-y-4">
                    <Button
                        variant="ghost"
                        asChild
                        className="w-fit cursor-pointer mb-0"
                    >
                        <Link href="/signup">
                ...
                <CardContent>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        {/* Campos de Organización (Solo para ADMIN) */}
                        {role === "ADMIN" && (
                            <>
                                <div className="space-y-2">
                                    <Label htmlFor="organizationName">Nombre de la Organización</Label>
                                    <Input id="organizationName" type="text" placeholder="Nombre de tu organización" {...register("organizationName")} />
                                    {errors.organizationName && <p className="text-sm text-destructive">{errors.organizationName.message}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="organizationAddress">Dirección de la Organización</Label>
                                    <Input id="organizationAddress" type="text" placeholder="Dirección de tu organización" {...register("organizationAddress")} />
                                    {errors.organizationAddress && <p className="text-sm text-destructive">{errors.organizationAddress.message}</p>}
                                </div>
                            </>
                        )}
