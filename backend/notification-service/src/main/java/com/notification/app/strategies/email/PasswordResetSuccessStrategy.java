...
public class PasswordResetSuccessStrategy implements EmailNotificationStrategy {
    
    private final EmailService emailService;

    @Override
    public NotificationType getType() {
        return NotificationType.PASSWORD_RESET_SUCCESS;
    }
    ...
    public void send(GenericNotificationDTO dto) {
        try {
            EmailRequest request = EmailRequest.builder()
                    ...
                    .supportEmail((String) dto.getVariables().get("supportEmail"))
            ...
            emailService.sendPasswordResetSuccessEmail(request);
        } catch (Exception e) {
            log.error("❌ Invalid variables for password reset success email: {}", e.getMessage(), e);
